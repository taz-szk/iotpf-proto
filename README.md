# MQTTでのデータ収集・可視化基盤
いわゆるioTデータと呼ばれるデータを簡易的にローカルで格納、可視化するアプリケーションです。
付随的にデバイスの管理としてブートストラップトークンを用いたゼロタッチプロビジョニング、OTAなども具備しています。
マルチテナントかつローカルでMQTTでのデータ送信におけるGrafanaでの可視化などに興味がある方は使ってみてください。
認証なしでアクセスできる公開可視ページの設定も可能なのでサイネージなどにも使えると思います。

インストールはインストールドキュメントをご参照ください。
claudeさんと一緒に作ったものなので、このリポジトリにあるコードやデータは、個人の責任において自由に利用して構いません。
一定のセキュリティ実装チェックなどは行っていますが、作者は一切の責任を負えませんのでその点ご了承ください。
より良い改修など自由に行っていただいて構いません。
...
The software and materials in this repository are provided for free use under your own responsibility. The author assumes no liability whatsoever.
